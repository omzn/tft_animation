...
void Character::setOrient(uint8_t o) { orient = o; }
...
int Character::update() {
  ...
  if (status != STATUS_STOP) {
    if (status == STATUS_MOVE) {
      if (direction == MOVE_UP) {
        tft->fillRect(pos_x, pos_y + 32 - move_diff, 32, move_diff,
                      ST7735_BLACK);
        pos_y -= move_diff;
        orient = ORIENT_BACK;
      } else if (direction == MOVE_DOWN) {
        ...
        tft->fillRect(pos_x + 32 - move_diff, pos_y, move_diff, 32,
                      ST7735_BLACK);
        pos_x -= move_diff;
        orient = ORIENT_LEFT;
      } else if (direction == MOVE_RIGHT) {
...
int Character::drawBitmap16(const unsigned char *data, uint8_t x, uint8_t y,
                            uint8_t w, uint8_t h) {
